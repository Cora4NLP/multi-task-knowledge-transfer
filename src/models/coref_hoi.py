...
def max_antecedent(top_span_emb, top_antecedent_emb, top_antecedent_scores, device):
    ...
    refined_span_emb = batch_select(top_antecedent_emb, max_antecedent_idx, device=device).squeeze(1)  # [num top spans, span emb size]
    return refined_span_emb
        ...
        if conf['fine_grained']:
            ...
            for depth in range(conf['coref_depth']):
                ...
                if conf['higher_order'] == 'cluster_merging':
                    cluster_merging_scores = cluster_merging(top_span_emb, top_antecedent_idx, top_pairwise_scores, self.emb_cluster_size, self.cluster_score_ffnn, None, self.dropout,
                    ...
                    if conf['higher_order'] == 'attended_antecedent':
                        refined_span_emb = attended_antecedent(top_span_emb, top_antecedent_emb, top_pairwise_scores, device)
                    elif conf['higher_order'] == 'max_antecedent':
                        refined_span_emb = max_antecedent(top_span_emb, top_antecedent_emb, top_pairwise_scores, device)
                    elif conf['higher_order'] == 'entity_equalization':
                        refined_span_emb = entity_equalization(top_span_emb, top_antecedent_emb, top_antecedent_idx, top_pairwise_scores, device)
                    elif conf['higher_order'] == 'span_clustering':
                        refined_span_emb = span_clustering(top_span_emb, top_antecedent_idx, top_pairwise_scores, self.span_attn_ffnn, device)
