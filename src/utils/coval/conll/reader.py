...
from src.utils.coval.conll import mention
...
def extract_annotated_parse(
    ...
    for i, line in enumerate(mention_lines):
        ...
        for j, c in enumerate(parse):
            if c == "(":
                if tag_started:
                    node = mention.TreeNode("".join(tag_name), pos_tags, start_index + i, False)
                    if open_nodes:
            ...
            if i == len(mention_lines) - 1 and j == len(parse) - 1 and terminal_nodes:
                node = mention.TreeNode(" ".join(terminal_nodes), pos_tags, start_index + i, True)
                if open_nodes:
                    if open_nodes[-1].children:
                        open_nodes[-1].children.append(node)
                    else:
...
def set_annotated_parse_trees(
    clusters, key_doc_lines, NP_only, min_span, partial_vp_chain_pruning=True, print_debug=False
):
...
def get_coref_infos(
    key_file, sys_file, NP_only=False, remove_nested=False, keep_singletons=True, min_span=False
):
