# taken from https://github.com/lxucs/coref-hoi/blob/master/preprocess.py
...
def minimize_partition(partition, extension, args, tokenizer):
    ...
    output_path = os.path.join(
        args.output_dir, f"{partition}.{args.language}.{args.seg_len}.jsonlines"
    )
    doc_count = 0
    ...
    with open(input_path, "r") as input_file:
        for line in input_file.readlines():
            ...
            if begin_document_match:
                doc_key = get_doc_key(begin_document_match.group(1), begin_document_match.group(2))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Input directory that contains conll files"
    )
    parser.add_argument("--output_dir", type=str, required=True, help="Output directory")
    parser.add_argument(
        "--seg_len", type=int, default=128, help="Segment length: 128, 256, 384, 512"
    )
    parser.add_argument("--language", type=str, default="english", help="english, chinese, arabic")
